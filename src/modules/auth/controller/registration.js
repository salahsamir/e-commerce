...
export const forget_password=asyncHandeler(
    async(req,res,next)=>{
        ...
        if(!user){
          return next(new Error('this email not exist',{cause:400}))
          }
...
export const change=asyncHandeler(
    async(req,res,next)=>{
        ...
        const user=await userModel.findOneAndUpdate({email:data.email},{password:hash_password,changePasswordTime:Date.now()},{new:true})
